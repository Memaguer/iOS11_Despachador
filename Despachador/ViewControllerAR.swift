...
class ViewControllerAR: UIViewController, ARSKViewDelegate, CLLocationManagerDelegate {
    ...
    func updateSites(){
        ...
        let urlString = "https://kuhni-fb8ab.firebaseio.com/buses.json"
    ...
    func createSites(){
        ...
        for index in 0...12 {
            ...
            let lat = sitesJSON["result"][index]["latitude"].doubleValue
            let lon = sitesJSON["result"][index]["longitude"].doubleValue
            ...
            print("placa: \(sitesJSON["result"][index]["licensePlate"].string!)")
            print("angulo: \(angle)")
            ...
            //let verticalRotation = simd_float4x4(SCNMatrix4MakeRotation(Float(distance/1000), 0, 1, 0))
            
            /*var verticalRot: Float
            if distance > 10000 {
                verticalRot = 0.1
            }
            ...
            print("clamp: \(clamp(value:distance / 2000, lower: -0.2, upper: 0.3)))")
            let verticalRotation = simd_float4x4(SCNMatrix4MakeRotation(clamp(value:distance / 100000, lower: -0.2, upper: 0.3), 0, 1, 0))
            ...
            translation.columns.3.z = -clamp(value:distance / 1500, lower: 0.2, upper: 4.0)
            
            let transform = simd_mul(rotation2, translation)
            
            let anchor = ARAnchor(transform: transform)
            sceneView.session.add(anchor: anchor)
            sites[anchor.identifier] = "\(sitesJSON["result"][index]["licensePlate"].string!) - \(Int(distance)) metros"
